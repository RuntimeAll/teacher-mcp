"""MCP 工具·题库录入组（data 角色）：
  转换层  convert_doc / convert_pdf / parse_paper_text（确定性预处理，无需 login）
  写层    format_question / upload_image / ingest_question / ingest_items / verify_ingest
  打标层  label_question

PRD-O-005 重建：合并旧 app/tools/{convert,ingest,label}.py 三文件；单 client（:9090）。
...
from teacher_mcp.domains.paperparse import (
    FIG, FIG_IDS, dedup_key, infer_type, plain_text, strip_source_prefix,
)
from teacher_mcp.tools.prep import _standard_scores

# 仓根 = src/teacher_mcp/tools/data_qbank.py 往上四层
...
class IngestItem(BaseModel):
    ...
    question_type: Optional[int] = Field(default=None, description="1-8（domains/dicts.py）；null=按内容推断")
...
def register(mcp, client: RuoyiClient) -> None:
    # ═════════════════════ 转换层（确定性预处理，无需 login）═════════════════════
    @mcp.tool(tags={"data", "ingest", "lecture"})
    ...
    async def ingest_items(
        ...
        参数:
          items: IngestItem[]（契约见 get_role_manual；未知字段拒绝，缺 stem 单条 fail 不中断批）
    ...
    async def _apply_scores(paper_id, ok_qids, total_score, suggest_time):
        """卷分值：items 带 score 则原样用（0 的小题默认 3）；全 0 则按年级标准分（_standard_scores）。"""
