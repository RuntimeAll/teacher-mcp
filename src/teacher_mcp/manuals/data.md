...
1d. 🔴 **学生可用性终验**（README §22，交付给学生打印前必过）：①图注泄漏终扫 `block_json LIKE '%（图中%'`=0（图注为题干唯一文字→剥壳转正文，透明底 PNG 判可见性须 alpha 合成白底）；②内部占位终扫（其一/问题N/作者：/版权声明段）=0，DB 正则全库扫别依赖抽查；③作答位完整（一句多空下划线齐全、看图列式有作答行、作答表格必须为空格）；④验收视角=学生书写/阅读规范优先于源书 1:1。
1c. 🔴 **选项与表格三闸**（README §21）：①拆选项**以形态判**（≥2 个"[A-D]．"，含一行多选项/单半角空格分隔），绝不拿 question_type 当闸（误标常见）；②图选项按源排布落 option 块——横排=同行多列，**图宽按 cell% 换算非页%**（页%直搬=竖排大图病）；③源 w:tbl **全量对账零静默**（纯文本表+空作答表都算；空作答表锚定走位置法非内容法，内容匹配对"只有表头"的空表必然歧义漏修）。
