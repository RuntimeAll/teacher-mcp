...
老师给你一个「帮 X 备某节课」的意图，你按**既定课程规划**，拿学情、圈题装题、题不够则补题、据讲义出题，最后组卷/组专项渲染成**可打印材料**交付。
...
3. **读课次蓝本**：`get_plan_detail(plan_id)` — 从 `lessons` 里找到本课次的 `segTemplate`（分几段、每段风格/分层规则）+ `kgNodeIds`（课内锚点）+ `layerTarget`。🔴 这是圈题的依据来源，跳过它就没法按段挑题。
4. **展开锚点（可选）**：`resolve_kg` / `list_kg_tree` 把 `kgNodeIds` 展开成知识点名 + 子树范围，确认圈题范围。
5. **圈题**：对每段，`search_questions(subject_id=课次锚点, difficult=按段分层规则, question_type?)` —
...
6. **题不够 → 补题**（两条路，择一或并用）：
   - **A · 举一反三变式**（agent 自造，不走 toolkit）：
     ...
     4. `search_questions(mine=True, subject_id=锚点)` 把刚造的题捞回来装段。
   - **B · 据讲义出题**：
...
7. **装包**：`build_prep_pack(session_id 或 lesson_id, segs=[{name, style, question_ids, rules, note, groups}])` — 每段填圈好/造好的 `question_ids`（🔴 字符串雪花号）。🔴 装包的 qid 必须是**真实存在**的题（圈的或造完捞回的），别塞臆造 id。
8. **交付**：`render_prep_pack(pack_id, mark_ready=True)` → 拿 PDF（单文件 A4，仅题目无解析，给学生打印）。
   - 要整卷/专项：`compose_paper(outline)`（按大纲确定性组卷落库）或 `create_paper(question_ids)` + `update_paper`。

---

## 🔴 铁律（违反 = 事故）

1. **私有池不 promote 不 set-public（版权红线）**：举一反三变式 / 据讲义自造的题一律留在**本人私有池**，圈回自有题用 `search_questions(mine=True)`。**绝不调 set-public**（那是超管审核动作）。备课自用完全够，公开是另一条审核链。
2. **家长可见产物无内部词**：`parentCopy` / 家长反馈 / 交给学生的 PDF 里**不出现** 层 / ★ / 素材 / 挑题 / 薄弱 等内部词。备课包 PDF 是题面本身，别混内部标注。
3. **写操作走登录身份**：所有落库（build_prep_pack / ingest_items / render）归属 = 登录老师，不伪造 owner、不传 createBy。
4. **题为锚，先看后装**：装段前 `get_question` 看清题面，别凭 id 盲装（装错题误人子弟）；变式/出题后自查答案。
5. **id 全字符串**：题 / 包 / 计划 / 课次 id 都是雪花号，全程按字符串传（JSON number 会截尾）。
6. **装包 qid 必须真实存在**：question_ids 里的每个 id 都得是题库里真有的题（圈的 / 造完 `mine=True` 捞回确认的），render 会按 qid 拉题面，塞了不存在的 id 段会渲染失败。
...
**问题**：`ingest_items` 落库的题（status / is_public 初始态）能否被 `search_questions(mine=True)` 检索到？能否被 `render_prep_pack` 正常读取 stem？变式路径需不需要额外 promote？
...
4. **render / get_question 能正常读 stem**。已发布题（status='1'）不被列表/详情过滤，装包渲染可读题面。实测通过。
...
| 装包 / 渲染 | `build_prep_pack` · `render_prep_pack` |
| 组卷交付 | `compose_paper` · `create_paper` · `update_paper` |
